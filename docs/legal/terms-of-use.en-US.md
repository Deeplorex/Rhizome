# Rhizome Terms of Use

Version 1.0 | Effective September 3, 2026

These terms apply to the Rhizome desktop application. By downloading, installing, or using the app, you acknowledge and accept these terms. If you do not agree, stop using and uninstall the app.

## 1. Purpose

Rhizome helps an individual manage long-lived accounts, keys, credential files, and their usage relationships. It does not provide cloud hosting, sign in on your behalf, browser autofill, automatic credential rotation, or publisher-assisted recovery.

## 2. License to use

Subject to these terms and applicable law, you may install and use Rhizome for personal purposes. Rights to copy, modify, or redistribute Rhizome itself are governed by any separate software license included with the release or published on the project page. Third-party components remain governed by their own open-source licenses.

## 3. Lawful and authorized use

- Store only accounts, credentials, and personal information that you are authorized to possess, manage, or use.
- Do not use the app for unauthorized access, circumvention of security controls, privacy violations, or unlawful conduct.
- Credentials for a third-party platform remain subject to that platform's terms.

## 4. Master password, recovery key, and device

You are responsible for protecting the master password, recovery key, vault, backups, and device. The publisher does not retain decryption material and cannot reset your master password or recover lost data. Do not keep the only copies of the recovery key and vault on the same device or disk.

## 5. Backups and verification

Create and test encrypted backups according to the importance of your data, with an additional backup before upgrades, migration, or bulk changes. Dependency maps and notes assist personal organization but do not replace actual system configuration, platform consoles, or a professional audit.

## 6. Security boundary

Local encryption, auto-lock, temporary reveal, and clipboard clearing reduce risk but cannot provide absolute security or defeat a compromised administrator account, malware, keylogging, screen capture, a weak master password, or unsafe backups.

## 7. App status and limitation of liability

To the fullest extent permitted by applicable law, the app is provided as is, without a promise that it will be uninterrupted, error-free, or suitable for every purpose. Responsibility for loss is determined under applicable law. Nothing here excludes liability that cannot lawfully be excluded or limited.

## 8. Updates and compatibility

You decide whether to install a new version. The publisher may change the app or these terms for security, compatibility, or functionality and will describe material changes in release notes. Cloud sync, telemetry, or automatic updates will not be enabled without clear notice.

## 9. Third-party software

Rhizome includes open-source software. Copyrights, licenses, and disclaimers remain with their respective owners and are listed under Third-party open-source licenses and in notices shipped with the release.

## 10. Stopping use

You may stop using and uninstall the app at any time. Before uninstalling, decide whether to retain an encrypted backup. You remain responsible for deleting vaults, backups, and exports stored in user-selected locations.

## 11. Applicable rules and contact

These terms are subject to applicable law. If one provision is invalid, the others remain in effect. The publisher identity and contact details are provided on the official Rhizome download page.

